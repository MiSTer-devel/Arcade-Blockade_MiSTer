...
void SimVideo::Clock(bool hblank, bool vblank, uint32_t colour) {
	...
	if (!(hblank || vblank)) {
		...
		// Write last pixel to texture
		output_ptr[last_address] = last_colour;

		// Generate next texture address
		uint32_t vga_addr = (y * xs) + x;

		// Write magenta to texture this time, schedule real pixel for next time
		output_ptr[vga_addr] = 0xFFFF00FF;
		last_address = vga_addr;
		last_colour = colour;
